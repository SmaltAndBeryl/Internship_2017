...
//        }
  ...
  $scope.gridOptions = {
    ...
    columnDefs: [
		{ name: 'Serial no'},
      	{ name: 'Institution name | Location', width: 300  },
		{ name: 'Type'},
		{ name: 'Date'},
		{ name: 'View Application'},
		{ name: 'Comments', enableCellEdit : true},
		{ name: 'Action', cellTemplate: '<button class="btn primary" ng-click="grid.appScope.deleteRow(row)">&#10004</button>' }
    ]
  };
  
  function refresh() {
    $http.get('classpath:sql/approveReject.yml')
